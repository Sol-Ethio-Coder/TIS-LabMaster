...
let SUPABASE_CONFIG = {
  url:
    ...
    "https://bxtequbmmtzfkgshmtqe.supabase.co",
  anonKey:
    localStorage.getItem("supabase_anon_key") ||
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ4dGVxdWJtbXR6Zmtnc2htdHFlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzk0ODQ3NTUsImV4cCI6MjA5NTA2MDc1NX0.85Wqg7kJqoAgZqlhNB6tbqnrtWGWgyB0a3AfCfD0GAI",
  table: "attendance_records",
};

// Initialize Supabase client
const supabaseUrl = "https://bxtequbmmtzfkgshmtqe.supabase.co";
...
function computeFinalTotal(componentScores, weights, finalExamScore) {
  ...
  for (let [comp, weight] of Object.entries(weights)) {
    ...
    if (!isNaN(val) && val !== "") courseworkSum += val * weight;
  }
  let examVal = parseFloat(finalExamScore);
  if (!isNaN(examVal) && examVal !== "") {
    return courseworkSum * 0.7 + examVal * 0.3;
  }
  return courseworkSum;
}
...
async function initSupabase() {
  if (typeof supabase === "undefined") {
    ...
    updateCloudStatusMessage(
      "⚠️ Supabase SDK not loaded - Please refresh the page",
      "error",
    );
    return false;
  }
  ...
  if (
    ...
    updateCloudStatusMessage(
      "⚠️ Click 'Configure Cloud' to set up Supabase",
      "warning",
    );
    return false;
  }
  ...
  try {
    ...
    if (error && error.message.includes("does not exist")) {
      updateCloudStatusMessage(
        "📦 Table not found - will create on first sync",
        "info",
      );
...
function configureCloud() {
  ...
  initSupabase().then(() => {
    if (isCloudConnected) {
      alert(
        "✅ Supabase configured successfully! Click 'Sync to Cloud' to backup your data.",
      );
    } else {
...
// Auto-sync function that works on every change
async function autoSyncToCloud() {
  console.log("🔵 autoSyncToCloud called - Checking conditions...");
  console.log("isCloudConnected:", isCloudConnected);
  console.log("supabaseClient:", !!supabaseClient);
  console.log("isLoggedIn:", isLoggedIn);

  if (!isCloudConnected || !supabaseClient || !isLoggedIn) {
    console.log("❌ Auto-sync skipped - conditions not met");
    return false;
  }
  ...
  try {
    ...
    console.log("Data size:", JSON.stringify(syncData).length, "bytes");
    console.log(
      "Sample - Grade8A students:",
      schoolData.term2?.grade8A?.students?.length,
    );
...
async function syncToSupabase() {
  if (!isCloudConnected || !supabaseClient) {
    updateCloudStatusMessage(
      "⚠️ Cloud not configured - click 'Configure Cloud' first",
      "warning",
    );
    return false;
  }
  ...
  try {
    ...
    updateCloudStatusMessage(
      "⚠️ Sync failed: " + (error.message || "Unknown error"),
      "error",
    );
    return false;
  }
  ...
  if (connected) {
    if (cloudStatus) {
      cloudStatus.innerHTML =
        '<i class="fas fa-cloud-upload-alt"></i> Cloud: Connected';
      cloudStatus.classList.add("connected");
    }
    if (cloudStorageStatus)
      cloudStorageStatus.innerHTML = "✅ Connected - Auto-save active";
    if (cloudStatusMsg)
      cloudStatusMsg.innerHTML =
        "☁️ Supabase cloud connected • Auto-saves on every change";
  } else {
    if (cloudStatus) {
      cloudStatus.innerHTML =
        '<i class="fas fa-cloud-upload-alt"></i> Cloud: Offline';
      cloudStatus.classList.remove("connected");
    }
    if (cloudStorageStatus)
      cloudStorageStatus.innerHTML = 'Not configured - Click "Configure Cloud"';
    if (cloudStatusMsg)
      cloudStatusMsg.innerHTML =
        '⚠️ Cloud not configured • Click "Configure Cloud" to enable auto-save';
  }
}
...
async function manualCloudSync() {
  ...
  if (result) {
    ...
    alert(
      "❌ Sync failed. Please check your Supabase configuration and try again.",
    );
  }
}
...
function renderMarklistHeader() {
  ...
  headerRow.innerHTML += `<th>Final Exam</th><th onclick="sortByColumn('finalTotal')">Final Total ⬍</th><th>Status</th>`;
  ...
  sortedStudents.forEach((student, idx) => {
    ...
    Object.keys(cls.weights).forEach((comp) => {
      ...
      inp.onchange = async (e) => {
        console.log("📝 Score changed for:", student.name, comp);
        let val =
          e.target.value === ""
            ? ""
            : Math.min(100, Math.max(0, parseFloat(e.target.value) || 0));
        student.componentScores[comp] = val;
        student.finalTotal = computeFinalTotal(
        ...
        renderMarklist();
        updateAnalytics();
        console.log("🔄 Calling autoSyncToCloud from score change...");
        const result = await autoSyncToCloud();
    ...
    if (student.finalTotal >= 80) remarkCell.innerHTML = "🏆 Excellent";
    else if (student.finalTotal >= 60) remarkCell.innerHTML = "✅ Good";
    else remarkCell.innerHTML = "📚 Needs Improvement";
  });
}
...
function renderAttendance() {
  ...
  filteredStudents.forEach((s, idx) => {
    let att = data.attendance[s.name] || {
      status: "absent",
      lastUpdated: new Date().toISOString(),
    };
    let row = tbody.insertRow();
    ...
    toggleBtn.onclick = async () => {
      ...
      schoolData[currentTerm][currentClassId].attendance[s.name] = {
        status: newStatus,
        lastUpdated: new Date().toISOString(),
      };
      persistToLocal();
...
function updateAttendanceStats() {
  ...
  let present = Object.values(data.attendance).filter(
    (a) => a.status === "present",
  ).length;
...
function updateAnalytics() {
  ...
  let median = sorted.length
    ? sorted[Math.floor(sorted.length / 2)].toFixed(1)
    : 0;
  let highest = scores.length ? Math.max(...scores).toFixed(1) : 0;
  let lowest = scores.length ? Math.min(...scores).toFixed(1) : 0;
  let passCount = scores.filter((s) => s >= 60).length;
  let passRate = scores.length
    ? ((passCount / scores.length) * 100).toFixed(1)
    : 0;
  let excellentCount = scores.filter((s) => s >= 80).length;
  let presentCount = Object.values(data.attendance).filter(
    (a) => a.status === "present",
  ).length;
  let attendanceRate = data.students.length
    ? ((presentCount / data.students.length) * 100).toFixed(1)
    : 0;
  let distribution = `${scores.filter((s) => s >= 80).length} A, ${scores.filter((s) => s >= 60 && s < 80).length} B, ${scores.filter((s) => s < 60).length} C`;
...
function generateReportCard(studentName) {
  ...
  for (let term of ["term1", "term2", "term3"]) {
    ...
    if (student) {
      termsHtml += `<tr><td>${termName}</td><td>${student.finalTotal.toFixed(1)}%</td><td>${student.finalExamScore || "--"}</td><td>${student.finalTotal >= 80 ? "Excellent" : student.finalTotal >= 60 ? "Good" : "Needs Improvement"}</td></tr>`;
    }
  }
  content.innerHTML = `
        <div class="report-card-print">
            ...
            <table border="1" style="width:100%; border-collapse: collapse;">
                <thead><tr><th>Term</th><th>Final Total</th><th>Final Exam</th><th>Status</th></tr></thead>
  ...
  for (let term of ["term1", "term2", "term3"]) {
    for (let cls of CLASSES) {
      ...
      if (data && data.students) {
        data.students.forEach((s) => {
          let att = data.attendance[s.name] || { status: "absent" };
          rows.push([
            term,
            cls.display,
            s.name,
...
function handleLogin() {
  ...
  if (
    ...
    const loginError = document.getElementById("loginError");
    if (loginError) {
      loginError.innerHTML = "❌ Invalid credentials! Use admin / admin123";
    }
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  console.log(
    "☁️ Supabase cloud storage ready - click 'Configure Cloud' to set up",
  );
});
